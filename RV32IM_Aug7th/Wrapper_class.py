...
class Wrapper(Elaboratable):
    def __init__(self):
        ...
        self.s_type = 0b011
        self.temp = Signal(32)
        # self.temp_arr = Array([Signal(signed(32)) for i in range(2**5)])
        # self.temp3 = Signal(10)
        self.reset = Signal(1)
    ...
    def elaborate(self,platform:Platform)->Module:
        ...
        # m.d.comb += ClockSignal().eq(clk100.i)
        # pos = ClockDomain("pos", async_reset=True)
        # neg = ClockDomain("neg",clk_edge="neg",async_reset=True)
        # neg.clk = pos.clk

        # # neg.clk = m.domains.sync.clk
        # m.domains += [neg]
        # m.d.neg += ClockSignal().eq(~clk100.i)
        ...
        with m.If(self.reset==Const(0)):
            ...
            with m.If(self.Busy[self.ID.s1] == Const(1)):
                with m.If(self.Busy1[self.ID.s1] == Const(0)):
                    m.d.sync += self.ID.s1_data_in.eq(self.ALU.result)
                with m.Else():
                    with m.If(self.ALU.inst_type!=0b011):
                        with m.If(self.ALU.inst_type!=0b100):
                            with m.If(self.ALU.reg_addr_out == self.ID.s1):
                                m.d.sync += self.ID.s1_data_in.eq(self.ALU.result)
                            with m.Else():
                                m.d.sync += self.ID.s1_data_in.eq(self.memory.data_out)
                        with m.Else(): 
                            m.d.sync += self.ID.s1_data_in.eq(self.memory.data_out)
            ...
            with m.If(self.Busy[self.ID.s2] == Const(1)):
                with m.If(self.Busy1[self.ID.s2] == Const(0)):
                    m.d.sync += self.ID.s2_data_in.eq(self.ALU.result)
                with m.Else():
                    with m.If(self.ALU.inst_type!=0b011):
                        with m.If(self.ALU.inst_type!=0b100):
                            with m.If(self.ALU.reg_addr_out == self.ID.s2):
                                m.d.sync += self.ID.s2_data_in.eq(self.ALU.result)
                            with m.Else():
                                m.d.sync += self.ID.s2_data_in.eq(self.memory.data_out)
            ...
            with m.If(self.Busy[self.ID.s1]==Const(0)):
                m.d.sync += self.ID.s1_data_in.eq(self.reg_file.write_Rs1_data)

            with m.If(self.Busy[self.ID.s2]==Const(0)):
                m.d.sync += self.ID.s2_data_in.eq(self.reg_file.write_Rs2_data)


            with m.If(self.Busy_csr[self.ID.csr_addr] == Const(1)):
                m.d.sync += self.ID.csrdata_in.eq(self.ALU.result)
            with m.Else():
                m.d.sync += self.ID.csrdata_in.eq(self.reg_file.write_csr_data)
...
#     m.d.sync += Wrapper1.reset.eq(reset)
